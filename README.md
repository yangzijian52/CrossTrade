...
CrossTrade 2.2.0 已于 2026-07-11 在 Paper 26.2 目标服务器完成实际加载和功能验收，服务器内实测通过。
...
4. 将 `CrossTrade-2.2.0.jar` 放入服务器 `plugins`。
5. 完整重启服务器。
...
最终 JAR：`target/CrossTrade-2.2.0.jar`。SQLite JDBC 被打入最终 JAR；Paper、Vault 和 Floodgate API 保持 provided。
